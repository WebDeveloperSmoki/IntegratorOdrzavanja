...
require("language/$choose_lang.php");
date_default_timezone_set("$default_time_zone");

echo "<html>
        <head>
            <title>".$lang['maintenanceDb']."</title>";
?>  
...
if(isset($_SESSION['timeout']) ) {
	...
	if($session_life > $session_timeout)
        { echo "<P>".$lang['LogintimeoutPlease']." <a href=\"logout.php\">".$lang['clickhere']."</a> ".$lang['forthemainpage'].".";
            die;
         }
}
...
if ((!isset($_SESSION["valid_user"])) && ($_SERVER['PHP_SELF'] != "$root_dir/login.php"))
{
    ...
    echo "<form method=\"post\"  action=\"$root_dir/advancedsearchresults.php?q=3\" style='margin: 0; padding: 0'><a href = \"$root_dir/dashboard.php\">".$lang['DashBoard']."</a> | <a href = \"$root_dir/searchresults.php\">".$lang['MyOrders']."</a>  | <a href = \"$root_dir/userprofile.php\">".$lang['MyProfile']."</a> | Search WO Summary: <input name=\"wosummary\" id=\"wosummary\" type=\"text\"><input name=\"wosummarybutton\" id=\"wosummarybutton\" type=\"submit\"> | logged in as $user [<a href = \"$root_dir/logout.php\">logout</a>]";
    ...
    if (is_dpt_admin($user) OR is_site_admin($user) OR is_plant_admin($user) OR is_system_admin($user))
        echo "[<a href = \"$root_dir/admin_modules/siteconfig.php\">".$lang['siteconfig']."</a>]</form>";
    else echo "</form>";
    }
}
